        ...
        <div class="registro grid md:grid-cols-5 md:gap-4 ml-5 mr-5">
            ...
            <div class="col-span-4 flex flex-col space-y-3">
                <div class="flex items-center justify-between">
                    <h3 class="nomecompleto__idade" id="nomeCompleto">
                        {{ editData.nome }}
                    </h3>
                    <div class="flex space-x-2">
                        <button @click="goToHomePage" class="button">Voltar à Página Inicial</button>
                        <button @click="showEditModal = true" id="btnEditarPerfil" class="button">Editar Perfil</button>
                    </div>
                </div>
                ...
                <div class="dados__pessoais grid grid-cols-2 gap-4 p-3 border rounded-lg shadow-md bg-white">
                    ...
                    <p class="dados__pessoais__bloco" id="dadoApelido">{{ editData.apelido }}</p>
                    <p class="dados__pessoais__bloco"><b>E-mail</b></p>
                    <p class="dados__pessoais__bloco" id="dadoEmail">{{ editData.email }}</p>
                    <p class="dados__pessoais__bloco"><b>Nascimento</b></p>
                    <p class="dados__pessoais__bloco" id="dadoNascimento">{{ editData.dataNascimento }}</p>
                    <p class="dados__pessoais__bloco"><b>CPF</b></p>
                    <p class="dados__pessoais__bloco" id="dadoCPF">{{ editData.cpf }}</p>
                    <p class="dados__pessoais__bloco"><b>Senha</b></p>
                    <p class="dados__pessoais__bloco">**********</p>
                    <p class="dados__pessoais__bloco"><b>Gênero</b></p>
                    <p class="dados__pessoais__bloco" id="dadoGenero">{{ editData.genero }}</p>
                    <p class="dados__pessoais__bloco"><b>Telefone</b></p>
                    <p class="dados__pessoais__bloco" id="dadoTelefone">{{ editData.telefone }}</p>
                </div>
                    ...
                    <form @submit.prevent="saveChanges">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="nomeCompletoEditar" v-model="editData.nome" class="input" />
                            </div>
                            <div>
                                <label class="block">E-mail</label>
                                <input type="text" id="emailEditar" v-model="editData.email" class="input" />
                            </div>
                            <div>
                                <label class="block">Apelido</label>
                                <input type="text" id="apelido" v-model="editData.apelido" class="input" />
                            </div>
                            <div>
                                <label class="block">Genero</label>
                                <input type="text" id="genero" v-model="editData.genero" class="input" />
                            </div>
                            <div>
                                <label class="block">CPF</label>
                                <input type="text" id="cpf" v-model="editData.cpf" class="input" />
                            </div>
                            <div>
                                <label class="block">Telefone</label>
                                <input type="text" id="telefone" v-model="editData.telefone" class="input" />
                            </div>
                        </div>
                        <button type="button" @click="showEditModal = false"
                            class="button mt-4 w-full bg-gray-500">Fechar</button>
                        <button type="submit" id="btnSalvarPerfil" class="button mt-2 w-full bg-blue-500">Salvar Alterações</button>
                    </form>
                </div>
            </div>
        </transition>
